...
{
	for (const auto body : bodies | std::views::values)
	{
		if (!body->IsDynamic()) continue; // Check if body is dynamic
//Downcast CollisionBody* to Rigidbody*
...
void DynamicsWorld::MoveBodies(const float deltaTime) const
{
	...
	for (const CollisionBody* body : bodies | std::views::values | std::views::filter(dynamics))
	{
		// ReSharper disable once CppCStyleCast
		const auto rigidbody = (Rigidbody*)body;
